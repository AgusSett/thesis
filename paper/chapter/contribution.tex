...
Primero se implementa la operación \func{rename} que dado una función de renombrado $\rho$ y un término, aplica el renombre de variables a dicho término.
...
De forma análoga, se define una función \func{exts} que extiende la sustitución cuando se atraviesa una abstracción.
...
Luego se define la substitución simple \func{$\_[\_]$}, que dato un término \bound{N} cuya primera variable libre es de tipo \bound{B} y un término \bound{M} de tipo \bound{B}, cierra el índice 0 de \bound{N} utilizando \bound{M}.

\ExecuteMetaData[code/subs.tex]{subst}

Notar las similitudes entre la definición de \func{rename} y \func{subst}, esto se debe a que los renombres son un caso particular de substitución, donde todos los términos retornados por la substitución son variables.
...
Una consecuencia directa de esto son los constructores con el prefijo \const{sym-}, todos estos se podrían obtener a partir de los isomorfismos base combinados con un constructor \const{sym} tal y como se construyen los isomorfismos de tipos.
...
La solución es definir nuevos constructores que embeben estas reglas, como por ejemplo \const{asso-split}.
...
Además, se utiliza \type{Maybe} en el tipo de retorno, en caso de que el término no converja a un valor luego de $n$ pasos, simplemente se retorna \const{nothing}.
...
Una substitución es adecuada cuando todos los términos por los cuales substituye las variables, cumple con el predicado \type{SN*}
...
Luego se utiliza la hipótesis \bound{Lƛ} en conjunto un nuevo lema.
...
El primer paso para extender la prueba, es agregar la definición de \type{SN} la relación de isomorfismos entre términos.
...
Por ejemplo, la prueba de \func{SN*-rename} ahora los dos constructores \const{inj₁} y \const{inj₂} del tipo suma.
...
\subst{\cons{\const{⟨} \func{rename} \bound{$\rho_1$} \bound{$u$} \const{,} \bound{$u_1$} \const{⟩}}{\comp{\ids}{\comp{\bound{$\rho_1$}}{\bound{$\rho$}}}}}{\bound{t}}
