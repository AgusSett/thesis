...
\begin{itemize}
	...
	\item El constructor \const{dist-ƛηₗᵣ} se aplica cuando se quiere mover las abstracciones fuera del producto, pero los elementos del par no tienen forma de lambda.
	...
	Cabe destacar que a nivel implementación, la $\eta$-expansión del término \bound{t} se escribe \const{ƛ}~\func{rename}~\const{S\_}~\bound{t}~\const{· ` Z}, donde el renombre suma uno a todas las variables libres, por lo que el índice 0 pasa a estar fresco en $t$.
