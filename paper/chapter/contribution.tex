...
Luego, la prueba de progreso consiste en probar la propiedad \type{Progress} se cumple para todo término dado.
...
La aplicación sigue la misma lógica, solo que en este caso se reduce primero la función y luego el argumento, cuando ambos términos llegan a una forma normal, se aplica la $\beta$-reducción.
...
Luego, para cada forma normal se deben tener en cuentas todos los isomorfismos que sean aplicables en cada caso.

Notar como solo es posible aplicar \const{step⇄ asso} cuando el elemento de la derecha es un par.
En caso de que dicho elemento esté en forma neutral, es necesario convertilo a un par aplicando la regla $r \rightleftarrows \langle \pi_A (r), \pi_B(r) \rangle$ antes de poder aplicar \const{asso}.
Por este motivo se añade el isomorfismo \const{asso-split} que combina ambas reglas en un solo paso.
