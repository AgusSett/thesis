...
\begin{center}
	\begin{prooftree}
		...
		\infer1[($\Rightarrow_i$)]{ \Gamma \vdash \lambda x^A.x : A \rightarrow A }
		\hypo{\Gamma \vdash t: B}
		\infer2[($\times_i$)]{ \Gamma \vdash \langle x^A.x, t \rangle : (A \rightarrow A) \times B }
		\infer1[($\forall_i$)]{ \Gamma \vdash \Lambda A . \langle x^A.x, t \rangle : \forall A. ((A \rightarrow A) \times B) }
		\infer1[($\equiv$)]{ \Gamma \vdash \Lambda A . \langle x^A.x, t \rangle : \forall A. (A \rightarrow A) \times \forall A.B }
		\infer1[($\times_e$)]{ \Gamma \vdash \pi_{\forall A.(A \rightarrow A)} (\Lambda A . \langle x^A.x, t \rangle) : \forall A.(A \rightarrow A) }
	\end{prooftree}
\end{center}
