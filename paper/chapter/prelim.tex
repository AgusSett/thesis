...
\begin{figure}[H]
	...
	\begin{prooftree}
		\hypo{A \equiv B}
		\infer1{A \times C \equiv B \times C}
	\end{prooftree}
	\quad
	\begin{prooftree}
		\hypo{A \equiv B}
		\infer1{C \times A \equiv C \times B}
	\end{prooftree}
	\quad
	\begin{prooftree}
		\hypo{A \equiv B}
		\infer1{A \rightarrow C \equiv B \rightarrow C}
	\end{prooftree}
	\quad
	\begin{prooftree}
		\hypo{A \equiv B}
		\infer1{C \rightarrow A \equiv C \rightarrow B}
	\end{prooftree}
...
\begin{figure}[H]
	...
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{r \rightleftarrows t}
	\end{prooftree} \quad
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{\lambda x.t \rightleftarrows \lambda x.r}
	\end{prooftree} \quad
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{ts \rightleftarrows rs}
	\end{prooftree} \quad
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{st \rightleftarrows sr}
	\end{prooftree} \\ \vspace{1em}
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{\langle t, s \rangle \rightleftarrows \langle r, s \rangle}
	\end{prooftree} \quad
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{\langle s, t \rangle \rightleftarrows \langle s, r \rangle}
	\end{prooftree} \quad
	\begin{prooftree}
		\hypo{ t \rightleftarrows r }
		\infer1{\pi_A t \rightleftarrows \pi_A r}
	\end{prooftree}
...
Donde $\mathbb{X}$ y $\mathbb{Y}$ son dos tipos no isomorfos, como por ejemplo $\tau$ y $\tau \rightarrow \tau$, e $y^\mathbb{X}$ es cualquier elemento de tipo $\mathbb{X}$.
...
Esto muestra que desde el punto de vista de la implementación, puede ser necesario agregar nuevas reglas de reescritura para evitar que aparezcan eliminaciones en formas normales.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		edge (m-2-3);
	\end{tikzpicture}
	\caption{El cubo lambda representa las dependencias entre tipos y términos como dimensiones ortogonales, las flechas corresponden a la relación ``incluido en''.}
\end{figure}
...
\begin{itemize}
	...
	Una prueba de la proposición $A \vee B$ consiste de una prueba de $A$ o una prueba de $B$ y un marcador que indica cuál de las dos alternativas está presente.
	...
	Los habitantes de $\Sigma x:A.B$ son pares dependientes $\langle a, b \rangle$ donde $a:A$ y $b:B(a)$.
	...
	Los habitantes de $\Pi x:A.B$ son funciones dependientes $a \rightarrow B(a)$.
