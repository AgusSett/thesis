...
\begin{document}
	...
	se codifica como $\langle \lambda x^1.r, \lambda x^2.s \rangle : 1 \Rightarrow A \times 2 \Rightarrow A$.
	Y la proyección $\pi_1 \langle r, s \rangle$ se codifica como $(\pi_{1 \Rightarrow A} \langle \lambda x^1.r, \lambda x^2.s \rangle) y^1$, donde 1 y 2 son dos tipos distintos, y $y^1$ es cualquier elemento de tipo 1. De forma análoga se codifica $\pi_2$.
	...
	\begin{itemize}
		\item Introducción de $\rightarrow$ e introducción de $\times$: $\lambda x.\langle r,s \rangle \leftrightarrow \langle \lambda x.r, \lambda x.s \rangle$
		...
		\item Eliminación de $\rightarrow$ e introducción de $\times$: $\langle r,s \rangle t \leftrightarrow \langle rt, st \rangle$
	...
	Hasta ahora se presentaron distintos tipos de cálculo lambda que podemos agrupar en dos categorías, el simplemente tipado ($\lambda_{\to}$), y el cálculo lambda polimórfico, también denominado \textit{Sistema F} o cálculo lambda de segundo orden ($\lambda 2$).
	...
	Notar como cada dependencia es, en cierta forma, ortogonal a las demás, y cada cálculo obtenido a medida que se añaden dependencias, es un superconjunto de los cálculos anteriores, es decir, se obtienen generalizaciones más grandes.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			(m-4-3) edge (m-3-4)
					edge (m-2-3);
		\end{tikzpicture}
		\caption{El cubo lambda representa las dependencias entre tipos y términos como dimensiones ortogonales, las flechas corresponde a la relación $\subsetneq$.}
	\end{figure}
	...
	\begin{itemize}
		...
		El cálculo $\lambda\omega$, que permite representar funciones polimórficas y constructores de tipos, es considerado como la base de muchos lenguajes funcionales.
	\end{itemize}
	
	El sistema que incluye a todos los demás, es denominado Cálculo de Construcciones ($\lambda C$), esta posee el mayor poder de cómputo y expresividad.
	...
	Por ejemplo, la propiedad de terminación es necesaria para que la lógica representada por el cálculo sea consistente.
